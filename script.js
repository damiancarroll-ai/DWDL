...
// ===== DWRDLE copy + keyboard mirroring helpers (non-invasive) =====
(function(){
  function elText(el){ return (el && (el.textContent||'').trim()) || ''; }
  function cellState(cell){
    ...
    const ds = (cell.getAttribute && (cell.getAttribute('data-state')||'').toLowerCase()) || '';
    if(ds==='correct' || ds==='present' || ds==='absent') return ds;
    return 'absent';
  }
  function squaresForState(st){
    const cb = document.body.classList.contains('colorblind');
    if(st==='correct') return cb ? '🟧' : '🟩';
    if(st==='present') return cb ? '🟦' : '🟨';
    return '⬛';
  }
  function findRows(){
    const board = document.getElementById('board');
    if(!board) return [];
    let rows = Array.from(board.querySelectorAll('.row, [data-row]'));
    if(rows.length===0){
      const tiles = Array.from(board.querySelectorAll('.cell, .tile, [data-letter]'));
      const grouped = [];
      for(let i=0;i<tiles.length;i+=5){
        grouped.push({fake:true, el:null, cells:tiles.slice(i,i+5)});
      }
      return grouped;
    }
    return rows.map(r=>({fake:false, el:r, cells:Array.from(r.querySelectorAll('.cell, .tile, [data-letter]'))}));
  }
  function rowTime(rowWrap){
    if(rowWrap.fake) return '';
    const r = rowWrap.el;
    const t = r.querySelector('.rowtime, [data-time]');
    return t ? elText(t) : '';
  }
  function copyGridAndTimes(){
    const rows = findRows();
    if(!rows.length){ alert('No board to copy'); return; }
    const lines = rows.map(wrap=>{
      const squares = (wrap.cells||[]).map(c=>squaresForState(cellState(c))).join('');
      const t = rowTime(wrap);
      return t ? (squares + '\\u2003' + t) : squares;
    });
    ...
    try{
      if(navigator.clipboard && navigator.clipboard.writeText){
        navigator.clipboard.writeText(payload).then(()=>{
          ...
          if(toast){ toast.textContent='Copied grid & times'; toast.classList.add('show'); setTimeout(()=>toast.classList.remove('show'), 1200); }
  ...
  document.addEventListener('click', function(e){
    ...
    if(btn){ e.preventDefault(); copyGridAndTimes(); }
  });
  const keyRank = {absent:1, present:2, correct:3};
  function keyForLetter(letter){
    const sel = `.key, button`;
    const all = Array.from(document.querySelectorAll(sel));
    const low = (letter||'').toLowerCase();
    for(const b of all){
      const dk = (b.dataset && (b.dataset.key||'')).toLowerCase();
      const txt = (b.textContent||'').trim().toLowerCase();
      if(dk===low || txt===low){ return b; }
    }
    return null;
  }
  function applyKeyState(letter, state){
    const key = keyForLetter(letter);
    if(!key) return;
  ...
  function scanBoardToKeys(){
    const rows = findRows();
    rows.forEach(wrap=>{
      (wrap.cells||[]).forEach(c=>{
        const st = cellState(c);
        const letter = elText(c).slice(0,1);
        if(letter){ applyKeyState(letter, st); }
      });
    });
  }
  const board = document.getElementById('board');
  if(board && window.MutationObserver){
    const mo = new MutationObserver(()=>scanBoardToKeys());
    mo.observe(board, {subtree:true, attributes:true, attributeFilter:['class','data-state']});
  }
  document.addEventListener('keydown', (e)=>{ if(e.key==='Enter'){ setTimeout(scanBoardToKeys, 10); } });
  document.addEventListener('click', (e)=>{
    const k = e.target && e.target.closest && e.target.closest('.key');
    if(k && (/enter/i.test(k.textContent))) setTimeout(scanBoardToKeys, 10);
  });
})(); // ===== end helpers =====
...
// --- DWORDLE v10.4: Exact two-space format + LF newlines ---
(function(){
  function brisbaneHeader(){
    try {
      const fmt = new Intl.DateTimeFormat('en-AU', { timeZone: 'Australia/Brisbane', year:'numeric', month:'short', day:'numeric' });
      const parts = fmt.formatToParts(new Date());
      const d = parts.find(p=>p.type==='day')?.value || '';
      const m = parts.find(p=>p.type==='month')?.value || '';
      const y = parts.find(p=>p.type==='year')?.value || '';
      return `DWORDLE — ${m} ${d}, ${y}`;
    } catch(e){
      const d = new Date().toLocaleDateString('en-AU', { month:'short', day:'numeric', year:'numeric' });
      return `DWORDLE — ${d}`;
    }
  }

  function rowToLine(rowEl){
    const cells = rowEl.querySelectorAll('.cell');
    let s = '';
    cells.forEach(cell => {
      if (cell.classList.contains('correct')) s += '🟩';
      else if (cell.classList.contains('present')) s += '🟨';
      else s += '⬛';
    });
    const t = rowEl.querySelector('.rowtime');
    s += '  ' + (t ? t.textContent.trim() : '00:00');
    return s;
  }

  function buildPayload(){
    const rows = document.querySelectorAll('.row');
    const lines = Array.from(rows).map(rowToLine);
    return [brisbaneHeader(), ...lines].join('\n');
  }
  ...
  function copyAll(){
    ...
    if (navigator.clipboard && navigator.clipboard.writeText){
      navigator.clipboard.writeText(text).then(()=>showToast('Copied grid & times!')).catch(()=>{fallbackCopy(text);showToast('Copied!');});
    } else {
      fallbackCopy(text);showToast('Copied!');
    }
  }
